...
1. ~~Resolve the one `KNOWN_ISSUE` — `3d/postprocessing-bloom`.~~ **DONE.** three.js is now loaded as an ES module via an importmap in `demo/index.html`; the `examples/jsm` postprocessing addons are imported in the bloom preview and attached to `window.THREE` (sharing one three instance), and a real `onResize`/`onTick`-before-`composer` ordering bug in the snippet — exposed once the addon actually loaded — was guarded. `KNOWN_ISSUES` is now empty; the whole vault passes the gate with no exclusions.
